...
"Memory`t`t`t: {0} GB used of {1} GB" -f ([Math]::Round($mem.TotalVisibleMemorySize / 1MB, 2)-[Math]::Round($mem.FreePhysicalMemory / 1MB, 2)),([Math]::Round($mem.TotalVisibleMemorySize / 1MB, 2))
